...
if blurf
    ...
    M.flt(2,4).kk = []
    % 
    M.flt(3,1).kk = unique([ceil(15465*r):ceil(18500*r)]); %Rodi
    M.flt(3,2).kk = unique([ceil(18500*r):ceil(24793*r)]); %Rodi
    M.flt(3,3).kk = unique([ceil(49000*r):ceil(50200*r)]); % betas S/L
    M.flt(3,4).kk = [];
    % 
    M.flt(4,1).kk = unique([ceil(18804*r):ceil(21908*r)]); %Rodi
...
grid
ss=sprintf('figs/Detrended-comps-%s-PS-%s.jpg',PRESSURE,ALL);
saveas(gcf,ss,'jpg')
...
grid
ss=sprintf('figs/Flow angles-%s-PS-%s.jpg',PRESSURE,ALL);
saveas(gcf,ss,'jpg')
...
title(ss)
grid
ss=sprintf('figs/Boxplot-factors-%s-PS-%s.jpg',PRESSURE,ALL);
saveas(gcf,ss,'jpg')
...
grid
ss=sprintf('figs/Boxplots_factors_all-%s-PS-%s.jpg',PRESSURE,ALL);
saveas(gcf,ss,'jpg')
